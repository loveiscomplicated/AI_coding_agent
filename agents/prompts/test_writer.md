...
`# TODO` 등 스켈레톤이 있는 기존 테스트 파일을 채울 때:

- **파일 전체를 새로 작성할 거면** `write_file` 로 완전히 덮어쓴다 (권장 기본값).
- **기존 import / fixture / 헬퍼를 보존하며 일부만 채울 거면**
  `hashline_read` → `hashline_edit` 순서로 정밀 편집.

빈 껍데기 (`# TODO` 그대로) 상태로 파일을 남겨두지 말 것.
