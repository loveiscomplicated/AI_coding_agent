...
- **파일 없음 → 즉시 작성**: `get_outline`, `read_file` 등이 에러를 반환하면(파일 없음, 경로 오류 등) 다른 경로로 탐색을 **계속하지 마세요**. 구현 전 TDD 단계이므로 참조 파일이 없는 것은 정상입니다. 태스크 설명·수락 기준에서 클래스명·메서드명을 직접 추론하여 **즉시 `write_file`을 호출**하세요.
