---
completeness: 98
hint: 5단계 전체 구현 완료. Phase 3 멀티 에이전트 및 모델 무관 설계까지 반영.
---
...
회의에서 context_doc.md 확정
  ...
  → LLM(LLM_MODEL_CAPABLE)이 context_doc.md를 읽고 tasks.yaml 초안 생성 (비동기)
...
결정 사항은 `data/context/decisions.md`에 자동 기록된다.
...
**저장 위치**: `data/reports/weekly/{year}-W{week}.md`
...
**저장 위치**: `data/reports/milestones/{timestamp}.md`  
**소비자**: 사람, 대시보드
...
| Discord Bot 기술 스택 | ✅ discord.py 기반 hotline/notifier.py 구현 완료 | 완료 |
