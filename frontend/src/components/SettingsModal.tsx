...
export function SettingsModal({ onClose }: Props) {
  ...
  const handleSelect = (model: ModelOption) => {
    setConvModel(model.id)
    setConvProvider(model.provider)
  }
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-xl w-[460px] max-w-[90vw] max-h-[80vh] flex flex-col border border-zinc-200 dark:border-zinc-700">
        {/* 헤더 */}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          ...
          {loading ? (
            <div className="text-xs text-zinc-400 dark:text-zinc-500">불러오는 중…</div>
          ) : models.length === 0 ? (
            <div className="text-xs text-zinc-400 dark:text-zinc-500">사용 가능한 모델이 없습니다.</div>
          ) : (
            <div className="space-y-4">
                ...
                <div key={provider}>
                  ...
                  <div className="space-y-1.5">
                    {grouped[provider].map(m => {
                      const selected = convModel === m.id && convProvider === m.provider
                      return (
                        <label
                          key={`${m.provider}:${m.id}`}
              ...
              {/* 현재 선택된 모델이 목록에 없는 경우 */}
              {convModel && !models.some(m => m.id === convModel) && (
                <div className="px-3 py-2 rounded-lg border border-amber-400 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-500 text-xs text-zinc-700 dark:text-zinc-300">
                  현재 설정: <span className="font-mono">{convProvider}/{convModel}</span>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-t border-zinc-200 dark:border-zinc-700 flex-shrink-0">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={handleSave}
              disabled={saving || loading || !convModel}
