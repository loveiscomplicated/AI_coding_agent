...
class TestResolveOrder:
    ...
    def test_complex_parallel_execution_groups(self):
        """
        복잡한 병렬 실행 그룹:
...
class TestHasCycle:
    ...
    def test_no_cycle_with_empty_dependencies(self):
        """
    ...
    def test_no_cycle_with_linear_dependencies(self):
        """
        선형 의존성 체인에는 순환이 없다
        """
        tasks = [
    ...
    def test_no_cycle_with_diamond_dependency(self):
        """
        다이아몬드 의존성에는 순환이 없다
        """
        tasks = [
    ...
    def test_cycle_simple_two_tasks(self):
        """
    ...
    def test_cycle_with_additional_tasks(self):
        """
        순환이 있으면서 다른 태스크도 있는 경우
        """
        tasks = [
            {"id": "task-001", "depends_on": []},
            {"id": "task-002", "depends_on": ["task-001"]},
            {"id": "task-003", "depends_on": ["task-002"]},
            {"id": "task-004", "depends_on": ["task-003", "task-002"]},  # 순환 없음
            {"id": "task-005", "depends_on": ["task-004"]},
            {"id": "task-006", "depends_on": ["task-005", "task-003"]},  # 순환 없음
        ]
        result = has_cycle(tasks)
        
        assert result is False

    def test_empty_task_list_no_cycle(self):
        """
        빈 태스크 리스트에는 순환이 없다
        """
        tasks = []
...
class TestValidateDependencies:
    ...
    def test_valid_dependencies_empty_list(self):
        """
    ...
    def test_invalid_multiple_tasks_with_missing_dependencies(self):
        ...
        # 두 개의 오류가 있어야 함
        assert any("task-999" in msg for msg in result)
        assert any("task-888" in msg for msg in result)
    ...
    def test_partial_invalid_dependencies(self):
        """
        일부 의존성만 유효하지 않은 경우
        """
        tasks = [
