...
[복잡도 평가 (complexity)]
각 태스크에 `complexity` 필드를 `simple` 또는 `non-simple` 중 하나로 평가하라.

### simple 조건 (전부 만족해야 함)
- target_files가 정확히 1개
- depends_on이 비어있음 (선행 태스크 없음)
- acceptance_criteria가 3개 이하
- description이 800자 이하

### non-simple
위 조건 중 하나라도 어긋나면 non-simple.

평가가 누락되거나 다른 값이면 시스템이 정량 지표로 자동 계산하므로 가능하면 명시하라.
`simple`이 아닌 경우는 모두 `non-simple`이다 — 세 단계로 나누지 않는다.
...
{"tasks": [{"id": "task-001", "title": "...", "description": "### 목적과 배경\\n...\\n\\n### 기술 요구사항\\n...\\n\\n### 인접 컨텍스트\\n...\\n\\n### 비고려 항목\\n...", "acceptance_criteria": ["..."], "target_files": ["Coordinate.kt", "Place.kt"], "depends_on": [], "task_type": "backend", "language": "kotlin", "complexity": "non-simple"}]}
"""

_CRITIQUE_SYSTEM_PROMPT = """\
당신은 소프트웨어 태스크 초안을 비판적으로 검토하는 전문가입니다.
응답은 반드시 JSON만 반환하세요. 마크다운 코드블록, 설명 텍스트 없이 순수 JSON만.

아래 CritiqueResult 구조를 그대로 출력하세요:
{
  "verdict": "APPROVED" | "NEEDS_REVISION",
  "summary": "전체 검토 요약 (1~3문장)",
  "issues": [
    {
      "task_id": "문제 있는 태스크 ID. 전체 구조 문제면 'GLOBAL'",
      "severity": "ERROR" | "WARNING",
      "category": "sizing" | "testability" | "dependency" | "scope" | "description",
      "message": "구체적 문제 설명"
    }
  ],
  "suggestions": ["개선 제안 (0~5개)"]
}

다음 기준을 이 순서로 검토하세요:

### 1. 스코프 (category: "scope")
- context_doc에서 명시적으로 결정된 기능만 태스크로 존재하는가
- 회의에서 결정되지 않은 기능이 추가됐으면 → ERROR
- context_doc에 있는 기능이 태스크에서 누락됐으면 → WARNING

### 2. 태스크 크기 (category: "sizing")
- target_files 3개 초과 → ERROR ("파일 3개 이하" 규칙)
- description이 단일 PR로 리뷰하기 어려운 수준이면 → WARNING

### 3. 테스트 가능성 (category: "testability")
- acceptance_criteria가 단위 테스트로 검증 가능한 형태인가
- "잘 동작해야 한다", "빠르게 처리해야 한다" 같은 모호한 기준 → ERROR
- 외부 시스템 의존 기준이 mock 없이 작성됐으면 → WARNING

### 4. 의존성 정합성 (category: "dependency")
- depends_on에 존재하지 않는 task_id → ERROR
- 순환 의존성 → ERROR
- 논리적으로 의존해야 할 것 같은데 depends_on 누락 → WARNING

### 5. description 품질 (category: "description")
- "목적과 배경 / 기술 요구사항 / 인접 컨텍스트 / 비고려 항목" 4개 섹션 헤더 누락 → WARNING
- description 100자 미만 → WARNING

### verdict 결정 규칙
- ERROR 하나라도 있으면 → NEEDS_REVISION
- WARNING만 있거나 없으면 → APPROVED (suggestions는 포함 가능)
"""
...
class CritiqueApplyRequest(BaseModel):
    tasks: list[dict]
...
def _run_draft(job_id: str, context_doc: str) -> None:
    ...
    try:
        ...
        for task in tasks:
            ...
            # complexity 검증 — binary(simple/non-simple) 정규화. 누락 시 자동 계산.
            from orchestrator.complexity import compute_complexity, normalize_complexity
            _VALID_COMPLEXITY = {"simple", "non-simple"}
            _task_id = task.get("id", "(id 없음)")
            _cx = task.get("complexity")
            _normalized = normalize_complexity(_cx)

            if _cx in ("standard", "complex"):
                # legacy 3단계 값 → non-simple 로 정규화
                task["complexity"] = "non-simple"
                warnings.append(
                    f"{_task_id}: complexity '{_cx}' → 'non-simple' 정규화 (binary 분류 체계)"
                )
            elif _normalized in _VALID_COMPLEXITY:
                # "simple"/"non-simple" 유효값 그대로
                task["complexity"] = _normalized
            else:
                # null/invalid → 자동 계산
                _computed = compute_complexity(task)
                task["complexity"] = _computed
                warnings.append(
                    f"{_task_id}: complexity 자동 계산 → {_computed}"
                )
...
def _run_critique(job_id: str, tasks: list[dict], context_doc: str) -> None:
    """백그라운드 스레드에서 태스크 초안 critique를 실행한다."""
    try:
        ...
        try:
            ...
            result = CritiqueResult(
                verdict="APPROVED",
                summary="critique 파싱 실패 (수동 검토 권장)",
                issues=[],
                suggestions=[],
            )

        with _critique_lock:
            _critique_jobs[job_id]["status"] = "done"
            _critique_jobs[job_id]["result"] = result.model_dump()

    except Exception as e:
        with _critique_lock:
            _critique_jobs[job_id]["status"] = "failed"
            _critique_jobs[job_id]["result"] = None


@router.post("/tasks/critique")
def start_critique(body: CritiqueRequest) -> dict:
    """
    태스크 초안을 LLM으로 비판적 검토한다. 백그라운드 실행, job_id 즉시 반환.
    GET /api/tasks/critique/{job_id} 로 상태 및 결과를 조회한다.
    """
    job_id = str(uuid.uuid4())
    with _critique_lock:
        _critique_jobs[job_id] = {"status": "running", "result": None}
    threading.Thread(target=_run_critique, args=(job_id, body.tasks, body.context_doc), daemon=True).start()
...
1. issues를 카테고리별로 처리한다:
   - dependency: depends_on 필드만 수정 (존재하지 않는 id 제거, 메시지에 언급된 누락 id 추가)
   - sizing: target_files가 3개 초과인 태스크를 분할하거나 target_files를 3개 이하로 줄임
   - testability: acceptance_criteria를 "동사+목적어+조건" 형식의 검증 가능한 문장으로 교체
...
def _parse_and_validate_apply_response(cleaned: str, original_tasks: list[dict]) -> dict:
    try:
        data: Any = json.loads(cleaned)
...
def apply_critique(body: CritiqueApplyRequest) -> dict:
    """Critique 결과를 기반으로 태스크 초안을 LLM으로 자동 수정한다."""
    tasks_json = json.dumps(body.tasks, ensure_ascii=False, indent=2)
    critique_json = json.dumps(body.critique.model_dump(), ensure_ascii=False, indent=2)
    user_content = (
        f"## 현재 태스크 목록\n\n{tasks_json}\n\n"
        f"## Critique 결과\n\n{critique_json}\n\n"
